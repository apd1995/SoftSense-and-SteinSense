...
def get_wavelets(original_data, wavelet_level):
    dat = np.transpose(original_data, (2, 0, 1))
...
def run_amp_instance(**dict_params):
    ...
    sparsified_array = get_sparsified_array(pooled_wavelets, prob_quantile)
    ...
    signal_true = sparsified_array_flat[:, :min(B, sparsified_array_flat.shape[1])]
    
    N = signal_true.shape[0]
