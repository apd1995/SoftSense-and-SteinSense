...
def seed(iter_count: int,
         ...
         sparsity_tol: float,
         mc: int) -> int:
    return round(1 + round(iter_count*1000) + round(nonzero_rows * 1000) + round(num_measurements * 1000) + round(signal_nrow * 1000) + round(signal_ncol * 1000) + round(err_tol * 100000) + round(sparsity_tol * 1000000) + round(mc * 10000))
...
def run_amp_instance(**dict_params):
    ...
    filename = dict_params['filename']
    sparsity = dict_params['sparsity']
    ...
    dict_params['signal_ncol'] = B
    dict_params['nonzero_rows'] = k
    dict_params['undersampling_ratio'] = delta
    
    rng = np.random.default_rng(seed=seed(iter_count, k, n, N, B, err_tol, sparsity_tol, mc))
    
    signal_true = np.array(df)
    signal_true = np.log2(signal_true+1)
    row_magnitudes = np.linalg.norm(signal_true, axis=1)
    threshold = np.quantile(row_magnitudes, 1 - sparsity)  # Magnitude threshold for top `q`
    signal_true[row_magnitudes < threshold] = 0.
...
def do_sherlock_experiment(json_file: str):
    ...
    nodes = 50
    with SLURMCluster(queue='donoho,hns,stat',
                      cores=1, memory='50GiB', processes=1,
                      walltime='5-00:00:00') as cluster:
...
if __name__ == '__main__':
    do_sherlock_experiment('exp_dicts/RNAseq_AMP_matrix_recovery_JS.json')
