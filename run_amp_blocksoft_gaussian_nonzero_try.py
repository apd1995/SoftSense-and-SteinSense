...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul  6 20:33:04 2025
...
def seed(gaussian_mean: float, 
    ...
    return round(1 + round(gaussian_mean * 100) + round(iter_count*1000) + round(nonzero_rows * 1000) + round(num_measurements * 1000) + round(signal_nrow * 1000) + round(signal_ncol * 1000) + round(err_tol * 100000) + round(mc * 100000) + round(sparsity_tol * 1000000))
...
def run_amp_instance(**dict_params):
    ...
    rng = np.random.default_rng(seed=seed(mu, iter_count, k, n, N, B, err_tol, mc, sparsity_tol))
    signal_true = np.zeros((N, B), dtype=float)
    nonzero_indices = rng.choice(range(N), k, replace=False)
    signal_true[nonzero_indices, :] = rng.normal(mu, 1, (k, B))
    ...
    while iter_count<max_iter and rel_err>err_tol and rel_err<err_explosion_tol:
        ...
        noise_cov_current = np.mean(np.var(Residual_current, axis = 0))*np.eye(B)
...
if __name__ == '__main__':
    do_sherlock_experiment('exp_dicts/AMP_matrix_recovery_blocksoft_gaussian_nonzero_try.json')
