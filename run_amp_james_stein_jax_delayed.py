...
def james_stein_nonsingular(X: np.ndarray, Sigma_inv: np.ndarray) -> np.ndarray:
    ...
    james_stein_coeff = jnp.where(quad_whitening > (d-2), 1 - ((d-2)/quad_whitening), 0.0)
    return X * james_stein_coeff[:, jnp.newaxis]
...
def james_stein_diagonal(X, diag_inv):
    d = X.shape[1]
    quad_whitening = jnp.sum(X**2 * diag_inv, axis=1)
    james_stein_coeff = jnp.where(quad_whitening > (d-2), 1 - ((d-2)/quad_whitening), 0.0)
...
def james_stein_singular(X, Sigma_eigvecs, nonzero_indices, Sigma_nonzero_eigvals_inv):
    # changing coordinates to get uncorrelated components
    X_indep = jnp.matmul(X, Sigma_eigvecs)
    ...
    # combine the two to get signal_newbasis
    signal_newbasis = jnp.zeros(X.shape, dtype = float)
    signal_newbasis[:, nonzero_indices] = signal_newbasis_indep
    signal_newbasis[:, zero_indices] = signal_newbasis_zero
    
    # we have identified U.T @ signal, now we need to get signal i.e revert to original coordinates
    signal_originalbasis = jnp.matmul(signal_newbasis, Sigma_eigvecs.T)
...
def james_stein_onsager_nonsingular(X, Z, Sigma_inv, rng, selected_rows_frac):
    ...
    dd_jacobian = dask.delayed(jax.jacrev)(james_stein_nonsingular_vec, argnums=0)
    jacobian_mat_list = [dd_jacobian(X[i,:], Sigma_inv) for i in selected_rows]
    sum_jacobians = dask.delayed(sum)(jacobian_mat_list)
    delayed_matmul = dask.delayed(np.matmul)
    onsager_term_delayed = delayed_matmul(Z, sum_jacobians.T).compute()
    return jax.device_get(onsager_term_delayed) / (Z.shape[0] * selected_rows_frac)
...
def james_stein_onsager_singular(X, Z, Sigma_eigvecs, nonzero_indices, Sigma_nonzero_eigvals_inv, rng, selected_rows_frac):
    ...
    Z = jnp.array(Z)
    selected_rows = rng.choice(X.shape[0], int(selected_rows_frac*X.shape[0]), replace = False)
    dd_jacobian = dask.delayed(jax.jacrev)(james_stein_singular_vec, argnums=0)
...
def read_and_do_local_experiment(json_file: str):
    exp = read_json(json_file)
    with LocalCluster(dashboard_address='localhost:8787', n_workers=20, threads_per_worker=2) as cluster:
    # with LocalCluster(dashboard_address='localhost:8787') as cluster:
        with Client(cluster) as client:
