...
def run_amp_instance(**dict_params):
    ...
    dict_params['collaboration_ratio'] = B/N
    ...
    while iter_count<max_iter and rel_err>err_tol and rel_err<err_explosion_tol:
        ...
        noise_var_current = np.var(Residual_current.flatten())
        # noise_cov_current_diag = np.diag(np.var(Residual_prev, axis = 0))
        
        noise_cov_current_inv = (1/noise_var_current)*np.eye(B)
...
if __name__ == '__main__':
    # do_local_experiment()
    read_and_do_local_experiment('exp_dicts/AMP_matrix_recovery_JS_normal_jit_diagonal.json')
    ...
    # do_sherlock_experiment('exp_dicts/AMP_matrix_recovery_JS_normal_jit_diagonal.json')
    # do_test_exp()
    # do_test()
